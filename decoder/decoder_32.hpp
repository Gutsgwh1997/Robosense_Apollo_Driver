  ...
  *azimuth = RS_SWAP_SHORT(mpkt_ptr->blocks[0].azimuth);
  ...
  for (size_t blk_idx = 0; blk_idx < this->lidar_const_param_.BLOCKS_PER_PKT;
    ...
    for (int channel_idx = 0;
      ...
      { // gwh changed，这一块要在point.set_timestamp(block_timestamp)之后
        uint64_t point_time = block_timestamp + 55520 * blk_idx + 2880 * (channel_idx % 16) + 1440 * std::floor(channel_idx / 16);
        setRingAndTimestamp(point, this->beam_ring_table_[channel_idx], point_time);
      }
